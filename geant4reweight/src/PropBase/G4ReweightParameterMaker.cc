...
G4ReweightParameterMaker::G4ReweightParameterMaker( const std::vector< fhicl::ParameterSet > & FitParSets, bool doProton, bool doNeutron ){

  std::vector< std::string > all_cuts;
  if( !doProton&&!doNeutron ){
    std::cout << "Doing pion" << std::endl;
    all_cuts= {"abs", "cex", "dcex", "prod", "inel", "reac"};
  }
  else if(doProton&&!doNeutron){
    std::cout << "Doing proton" << std::endl;
    all_cuts = {"total","reac"};
  }
  else if(doNeutron&&!doProton){
  std::cout << "Doing neutron" << std::endl;
    all_cuts = {"piprod", "abs", "pprod","inel"};
  }
